...
| Phillips curve (inflation) | Ball (1999) with exchange rate passthrough; McCarthy (1999) |
...
```
r_t = rho * r_{t-1} + (1 - rho) * [r_star + 1.5*(pi_t - pi_star) + k*y_t]
```
...
6. **AR(1) suboptimality** -- AR(1) baseline underperforms oracle by >3500% MSE on all variables.
