...
RESULTS_DIR = os.path.join(project_root, "data", "pilot_results", "weight_class")
...
def main() -> None:
    ...
    for model_cfg in PHASE2_MODELS:
        ...
        logger.info(
            f"Parse stats for {model_cfg['display_name']}: "
            f"{total} calls, {fails} parse failures, {errors} API errors"
        )
