...
def run_counterfactual(
    ...
    Returns:
        3D numpy array of shape (n_paths, n_forward_periods, 5).
        The 5 columns are [y, pi, r, e, u] in that order.
    """
    # Extract the final state from history to use as the branching point
    last_row = history_df.iloc[-1]
    # We also need e_prev (the exchange rate one period before the last)
    second_to_last_row = history_df.iloc[-2]

    last_period = int(last_row["period"])

    # Build the initial state for forward simulation
    branch_state = {
        "period": last_period,
        "y": float(last_row["y"]),
        "pi": float(last_row["pi"]),
        "r": float(last_row["r"]),
        "e": float(last_row["e"]),
        "e_prev": float(second_to_last_row["e"]),
        "u": float(last_row["u"]),
        "eps_y": 0.0,
        "eps_pi": 0.0,
        "eps_e": 0.0,
    ...
    # Allocate output array: (n_paths, n_forward_periods, 5 variables)
    ...
    for path_index in range(n_paths):
        ...
        sim = Sim(config_path, seed=base_seed + path_index)
